...
t1 <- table_data %>%
  dplyr::select(-gradeyear_binned, -ethnicity, -postcode_povertycoded)%>%
  tbl_summary(
    by = pronouns_binned,
    type = type,
    missing = "no",
...
t2 <- table_data %>%
  dplyr::select(-pronouns_binned, -ethnicity, -postcode_povertycoded)%>%
  tbl_summary(
    by = gradeyear_binned,
    type = type,
    missing = "no",
...
t3 <- table_data %>%
  dplyr::select(-pronouns_binned, -gradeyear_binned, -postcode_povertycoded)%>%
  tbl_summary(
    by = ethnicity,
    type = type,
    missing = "no",
...
t4 <- table_data %>%
  dplyr::select(-pronouns_binned, -gradeyear_binned, -ethnicity)%>%
  tbl_summary(
...
```{r}
data$nature <- as.factor(data$nature)
data$nature_simple <- as.factor(data$nature_simple)
#data$nature_simple <- as.factor(data$nature_simple)
data$friends_inperson <- as.factor(data$friends_inperson)

#data %<>% drop_na(cantril_prepandemic, edinborough_scale)
...
#df <- data %>%
  #filter(pronouns_binned %in% c("he/him", "she/her", "NB/pan"))%>%
  #drop_na(nature, friends_inperson)
```
...
p <- df %>%
  ggplot()+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=as.factor(closetoppl), alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = as.factor(closetoppl), size = 1), alpha = 0.2, lineend = "round", 
               data = df %>% filter(closetoppl %in% c(3,4,5)),
               show.legend = F)+
  stat_smooth(data=within(df, nature_simple <- NULL),
              aes(x=cantril_prepandemic, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=F)+
  scale_colour_manual(values = c("grey", "grey", "grey", "blue4", "blue4", "blue4"))+
  scale_x_continuous(limits = c(0,10), breaks = c(1:10))+
  guides(alpha = "none", size = "none", colour = "none")+
  scale_y_continuous(limits = c(5,35))+
  labs(x = "Pre-COVID Life Satisfaction", y = "Resilience Score")+
  facet_wrap(~nature_simple)

p
#ggsave("cantril_prepandemic-edinborough_scale.jpg", width = 8, height = 6, units = "in")
```
...
p1 <- df %>%
  ggplot()+
  stat_smooth(data=within(df, friends_online <- NULL),
              aes(x=cantril_prepandemic, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=F)+
  scale_x_continuous(limits = c(0,10), breaks = c(1:10))+
  guides(alpha = "none", size = "none", colour = "none")+
  scale_y_continuous(limits = c(5,35))+
...
p2 <- p1 +
  #geom_point(data = df %>% filter (nature %in% c("0","1", "2", "3")), aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=nature, alpha=0.2, size = 1))+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=nature, alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = nature, size = 1), alpha = 0.2, lineend = "round", 
               data = df %>% filter(nature %in% c(4,5)),
               show.legend = F)+
  scale_colour_manual(values = c("grey", "grey", "grey", "grey", "#20963e", "#20963e"))+
  facet_wrap(friends_inperson~friends_online)
p2
#ggsave("nature_hi-v-lo.jpg", width = 8, height = 6, units = "in")

p3 <- p1 +
  #geom_point(data = df %>% filter (nature %in% c("0","1", "2", "3")), aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=nature, alpha=0.2, size = 1))+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=as.factor(friends_inperson), alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = friends_inperson, size = 1), alpha = 0.2, lineend = "round", 
               data = df %>% filter(friends_inperson %in% c(2,3,4,5)),
               show.legend = F)+
    scale_colour_manual(values = c("grey", "grey", "grey", "#20963e", "#20963e", "#20963e"))+
  facet_wrap(~friends_online)
p3
#ggsave("time-with-friends.jpg", width = 8, height = 6, units = "in")

```

```{r}
df %>%
  drop_na(nature, friends_inperson_simple)%>%
  ggplot()+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=nature_simple, alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = nature_simple, size = 1), alpha = 0.2, lineend = "round", 
               data = df %>% filter(nature %in% c(4,5)),
               show.legend = F)+
  #geom_hline(aes(yintercept = 20))+
  stat_smooth(data=within(df, friends_inperson_simple <- NULL),
              aes(x=cantril_prepandemic, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=F)+
  scale_colour_manual(values = c("grey", "#20963e"), name = "Nature", labels = c("Low", "High"))+
  scale_x_continuous(limits = c(0,10), breaks = c(1:10))+
  guides(alpha = F, size = F)+
  labs(x = "Pre-COVID Life Satisfaction", y = "Resilience Score")
#ggsave("nature.jpg", width = 8, height = 6, units = "in")
```
...
df %>%
  drop_na(nature, friends_inperson_simple)%>%
  ggplot()+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=as.factor(friends_inperson_simple), alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = as.factor(friends_inperson_simple), size = 1), alpha = 0.2, lineend = "round", 
               data = df %>% filter(friends_inperson_simple == 1),
               show.legend = F)+
  #geom_hline(aes(yintercept = 20))+
  stat_smooth(data=within(df, nature_simple <- NULL),
              aes(x=cantril_prepandemic, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=F)+
  ...
  labs(x = "Pre-COVID Life Satisfaction", y = "Resilience Score")+
  facet_wrap(~nature_simple)
ggsave("nature_friends_inperson.jpg", width = 12, height = 6, units = "in")
```
...
```{r}
df %>%
  drop_na(friends_inperson, nature_simple)%>%
  ggplot()+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=nature, alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = nature, size = 1), alpha = 0.2, lineend = "round", 
...
```{r}
data$talktofamily_inhome  <- as.factor(data$talktofamily_inhome )

df <- data %>%
  filter(pronouns_binned %in% c("he/him", "she/her", "NB/pan"))

df %>%
  drop_na(talktofamily_inhome)%>%
  ggplot()+
  geom_point(aes(x=cantril_prepandemic + jit, y=edinborough_scale, colour=talktofamily_inhome , alpha=0.2, size = 1))+
  geom_segment(aes(x=cantril_prepandemic + jit, y=edinborough_scale, xend=cantril_prepandemic+jit, yend=predicted+jit, 
                   colour = talktofamily_inhome , size = 1), alpha = 0.2, lineend = "round", 
               data = df %>% filter(talktofamily_inhome  %in% c(5)),
               show.legend = F)+
  stat_smooth(aes(x=cantril_prepandemic, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=F)+
  scale_colour_manual(values = c("grey", "grey", "grey", "grey", "grey", "#20963e"))+
  scale_x_continuous(limits = c(0,10), breaks = c(1:10))+
  guides(alpha = F, size = F)

```

Probably shouldn't be doing a PCA for categorical variables
```{r}
...
p1 <- df_pca %>%
  ggplot()+
  geom_point(aes(x=axis1 , y=edinborough_scale, alpha=0.2, size = 1))+
  stat_cor(aes(x=axis1, y=edinborough_scale), label.y = 30)+
  stat_smooth(aes(x=axis1, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=T)+
  #scale_colour_manual(values = c("grey", "grey", "grey", "blue4", "blue4", "blue4"))+
  #scale_x_continuous(limits = c(0,10), breaks = c(1:10))+
  guides(alpha = "none", size = "none", colour = "none")+
  scale_y_continuous(limits = c(5,35))+
  labs(x = paste("PCA 1 (", round(s$importance[2]*100, 1), "%)", sep = ""), y = "Resilience Score")
  
p1

p <- df_pca %>%
  ggplot()+
  geom_point(aes(x=axis2 , y=edinborough_scale, colour=as.factor(nature_simple), alpha=0.2, size = 1))+
  stat_cor(aes(x=axis2, y=edinborough_scale), label.y = 30)+
  stat_smooth(aes(x=axis2, y=edinborough_scale), linetype = "dotted", colour = "black", method=lm, se=T)+
  #scale_colour_manual(values = c("grey", "grey", "grey", "blue4", "blue4", "blue4"))+
  #scale_x_continuous(limits = c(0,10), breaks = c(1:10))+
  guides(alpha = "none", size = "none")+
  scale_y_continuous(limits = c(5,35))+
  labs(x = paste("PCA 2 (", round(s$importance[5]*100, 1), "%)", sep = ""), y = "Resilience Score")
  
p

library(cowplot)
cowplot::plot_grid(p1, b1, ncol = 1)
ggsave("pca_with_loadings.jpg", width = 6, height = 12, units = "in")
